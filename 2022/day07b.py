...
if __name__ == "__main__":
    ...
    DISK_SIZE = 70000000
    NEEDED = 30000000
    dir_sizes = sorted([size for name, size in c.disk.du()])
    used_space = c.disk.rootdir.size
    free_space = DISK_SIZE - used_space
    for i in dir_sizes:
        if i > (NEEDED - free_space):
            print(f"The size of the smallest directory that can be removed to provide enough free space is {i}.")
            break

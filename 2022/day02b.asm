...
;; Figure out what hand we should play based on the opposing hand and the desired outcome
;; ON ENTRY:	ax contains other player's hand in ah and the desired outcome (win/lose/tie) in al
;; ON EXIT:	the hand we should play is returned in al, ah is preserved
determine_move:	cmp	al, 1				; we should lose
		je	.lose
		cmp	al, 2				; we should tie
		je	.tie
		cmp	al, 3				; we should win
		je	.win
		jmp	error_exit			; we should never reach here
.lose:		mov	al, ah				; copy opponent's move
		dec	al				; be the loser
		cmp	al, 1				; if less than one, change to 3
		jge	.done
		add	al, 3
		jmp	.done
.tie:		mov	al, ah				; just copy the opponent's move to tie
		jmp	.done
.win:		mov	al, ah				; copy opponent's move
		inc	al				; be the winner
		cmp	al, 3				; if less more than 3, change to 1
		jle	.done
		sub	al, 3
.done:		ret
...
score_round:	call	determine_move			; figure out what move to play based on other hand and instruction
		cmp	ah, al				; if both hands are equal, it's a tie
		je	.tie
		inc	ah

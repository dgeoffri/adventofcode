...
def display_output_lines(signal_generator, width=40, height=6):
    for y in range(height):
        line = []
        for x in range(width):
            cpu_x = next(signal_generator)
            sprite_position = range(cpu_x - 1, cpu_x + 2)
            if x in sprite_position:
                line.append("#")
            else:
                line.append(".")
        yield line
...
if __name__ == "__main__":
    ...
    for line in display_output_lines(cpu.cycle_accurate_X()):
        print(''.join(line))

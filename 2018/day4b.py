...
for minute in range(60):
	for guard in guardletters.values():
		guardsleptthisminute = 0
		for day in days:
			schedule = days[day]
			if schedule[minute] == guard:
				 guardsleptthisminute += 1
		tally.append((guardsleptthisminute, guard, minute))
for guardsleptthisminute, guard, minute in sorted(tally):
	print guard + " (" + guardnumbers[guard] + ") slept " + str(guardsleptthisminute) + " days at " + str(minute)

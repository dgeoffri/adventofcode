...
struct Coord * marchCars(struct Car * cargrid[150][151], char grid[150][151]) {
	struct Car * car;
	struct Coord * collision;
	int x, y;
	for (y=0; y<150; y++)
		for (x=0; x<150; x++)
			if (cargrid[y][x])
		...
		for (x=0; x<150; x++) {
			if ((cargrid[y][x]) && !(cargrid[y][x]->alreadyMoved)) {
				...
				if (cargrid[ynew][xnew]) {
					collision = malloc(sizeof(struct Coord));
					collision->x = xnew;
					collision->y = ynew;
					return collision;
				} else {
...
int main(int argc, char **argv) {
	...
	//	memcpy (destgrid, grid, sizeof(grid));
	// }
	while (!(collisionPoint = marchCars(cargrid, grid))) {
		// showCars(cargrid);
		memcpy (destgrid, grid, sizeof(grid));
		plotCars(cargrid, destgrid);
		snprintf(outfilename, sizeof(outfilename), "day13a-frame%08d.txt", i++);
	...
	printf ("There was a crash!  Carts collided at %d,%d\n", collisionPoint->x, collisionPoint->y);
}

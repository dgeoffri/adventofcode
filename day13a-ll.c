...
struct Car * newCar(int x, int y, int dir, int nextTurn, struct Car * oldCar) {
	struct Car * newcar;
	...
	for (y=0; y<150; y++) {
		for (x=0; x<150; x++) {
			switch (grid[y][x]) {
				case '^':
					grid[y][x] = '|';
					carsHead = newCar(x, y, 0, 0, carsHead);
					break;
				case '>':
					grid[y][x] = '-';
					carsHead = newCar(x, y, 1, 0, carsHead);
					break;
				case 'v':
					grid[y][x] = '|';
					carsHead = newCar(x, y, 2, 0, carsHead);
					break;
				case '<':
					grid[y][x] = '-';
					carsHead = newCar(x, y, 3, 0, carsHead);
					break;
			}
		}
		grid[y][150] = 0;
